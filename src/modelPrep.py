...
def createTrans(sparkDF):
	...
	sparkDFTrans = sparkDFTrans.select('*'
		, when(sparkDFTrans.NumberOfDependents.isNull(), 1).otherwise(0).alias("UnknownNumberOfDependents")
	)
	#x$UnknownMonthlyIncome <- ifelse(isNull(x$MonthlyIncome), 1, 0)
	sparkDFTrans = sparkDFTrans.select('*'
		, when(sparkDFTrans.MonthlyIncome.isNull(), 1).otherwise(0).alias("UnknownMonthlyIncome")
	)
	
	#x$NoDependents <- ifelse(x$NumberOfDependents == 0, 1, 0)
	sparkDFTrans = sparkDFTrans.select('*'
		, when(sparkDFTrans.NumberOfDependents == 0, 1).otherwise(0).alias("NoDependents")
	)
	#x$NumberOfDependents <- ifelse(x$UnknownNumberOfDependents==1, 0, x$NumberOfDependents)
	sparkDFTrans = sparkDFTrans.select('*'
		, when(sparkDFTrans.UnknownNumberOfDependents == 1, 0).otherwise(sparkDFTrans.NumberOfDependents).alias("NumberOfDependents")
	)
	...
	"""
	sparkDFTrans = sparkDFTrans.select('*'
		, when(sparkDFTrans.MonthlyIncome == 0 | sparkDFTrans.MonthlyIncome.isNull(), 1).otherwise(0).alias("NoIncome")
	)
	"""

	#x$MonthlyIncome <- ifelse(x$UnknownMonthlyIncome==1, 0, x$MonthlyIncome)
	sparkDFTrans = sparkDFTrans.select('*'
		, when(sparkDFTrans.UnknownMonthlyIncome == 1, 0).otherwise(sparkDFTrans.MonthlyIncome).alias("MonthlyIncome")
	)
	#x$ZeroDebtRatio <- ifelse(x$DebtRatio == 0, 1, 0)
	sparkDFTrans = sparkDFTrans.select('*'
		, when(sparkDFTrans.DebtRatio == 0, 1).otherwise(0).alias("ZeroDebtRatio")
	)
	#x$UnknownIncomeDebtRatio <- x$DebtRatio
	#sparkDFTrans = sparkDFTrans.select('DebtRatio').alias("UnknownIncomeDebtRatio")
	
	#x$UnknownIncomeDebtRatio <- ifelse(x$UnknownMonthlyIncome == 0, 0, x$UnknownIncomeDebtRatio)
	sparkDFTrans = sparkDFTrans.select('*'
		, when(sparkDFTrans.DebtRatio == 0, 0).otherwise(sparkDFTrans.DebtRatio).alias("UnknownIncomeDebtRatio")
	)
	#x$DebtRatio <- ifelse(x$UnknownMonthlyIncome == 1, 0, x$DebtRatio)
	sparkDFTrans = sparkDFTrans.select('*'
		, when(sparkDFTrans.UnknownMonthlyIncome == 1, 0).otherwise(sparkDFTrans.DebtRatio).alias("DebtRatio")
	)
